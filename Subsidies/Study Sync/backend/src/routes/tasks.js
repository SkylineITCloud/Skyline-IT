...
router.post(
  ...
  (req, res) => {
    ...
    const id = uuidv4();
    db.qRun(
      'INSERT INTO tasks (id, group_id, title, assigned_to, created_by) VALUES (?, ?, ?, ?, ?)',
      [id, group_id, title, assigned_to || null, req.userId]
    );
...
router.put('/:id', (req, res) => {
  ...
  if (completed !== undefined) {
    db.qRun('UPDATE tasks SET completed = ? WHERE id = ?', [completed ? 1 : 0, req.params.id]);
  }
  if (emoji !== undefined) {
    db.qRun('UPDATE tasks SET emoji = ? WHERE id = ?', [emoji, req.params.id]);
  }
