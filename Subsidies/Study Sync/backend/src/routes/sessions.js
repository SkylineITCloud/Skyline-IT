...
router.get('/', (req, res) => {
  ...
  const result = sessions.map(s => {
    const attendees = db.q(
      'SELECT u.id, u.name FROM session_attendees sa JOIN users u ON u.id = sa.user_id WHERE sa.session_id = ?',
      [s.id]
    );
    return { ...s, attendees };
  });
...
router.post(
  ...
  (req, res) => {
    ...
    const id = uuidv4();
    db.qRun(
      'INSERT INTO sessions (id, group_id, name, session_date, session_time, duration, type, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
    ...
    members.forEach(m => {
      db.qRun('INSERT INTO notifications (id, user_id, type, title, message) VALUES (?, ?, ?, ?, ?)', [uuidv4(), m.user_id, 'info', 'New session scheduled', `${name} on ${session_date} at ${session_time}`]);
    });
