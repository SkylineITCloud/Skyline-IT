...
router.get('/', (req, res) => {
  ...
  for (const g of groups) {
    ...
    const members = db.q(
      'SELECT u.id, u.name, u.is_online, u.last_seen FROM group_members gm JOIN users u ON u.id = gm.user_id WHERE gm.group_id = ?',
      [g.id]
    );
    result.push({
      id: g.id,
      name: g.name,
      course: g.course,
      study_method: g.study_method,
      frequency: g.frequency,
      max_members: g.max_members,
      member_count: count ? count.cnt : 0,
      members,
      is_member: !!g.member_id,
...
router.get('/mine', (req, res) => {
  const groups = db.q(
    'SELECT g.* FROM groups_ g JOIN group_members gm ON gm.group_id = g.id WHERE gm.user_id = ? ORDER BY g.created_at DESC',
    [req.userId]
  );
...
router.get('/:id', (req, res) => {
  ...
  const members = db.q(
    'SELECT u.id, u.name, u.total_hours, u.is_online, u.last_seen, gm.role, gm.joined_at FROM group_members gm JOIN users u ON u.id = gm.user_id WHERE gm.group_id = ?',
    [group.id]
  );
...
router.post('/:id/join', (req, res) => {
  ...
  if (count && count.cnt >= group.max_members) {
  ...
  members.forEach(m => {
    db.qRun('INSERT INTO notifications (id, user_id, type, title, message) VALUES (?, ?, ?, ?, ?)', [uuidv4(), m.user_id, 'info', 'New member joined', `${user.name} joined ${group.name}`]);
  });
