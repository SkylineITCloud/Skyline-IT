...
router.get('/', (req, res) => {
  ...
  const messages = db.q(
    `SELECT m.*, u.name AS user_name FROM messages m JOIN users u ON u.id = m.user_id WHERE m.group_id = ? ORDER BY m.created_at ASC LIMIT ?`,
    [group_id, numLimit]
  );
...
router.post(
  ...
  (req, res) => {
    ...
    const id = uuidv4();
    db.qRun(
      'INSERT INTO messages (id, group_id, user_id, content, type) VALUES (?, ?, ?, ?, ?)',
      [id, group_id, req.userId, content, type || 'text']
    );

    const messages = db.q(
      'SELECT m.*, u.name AS user_name FROM messages m JOIN users u ON u.id = m.user_id WHERE m.id = ?',
      [id]
    );
...
router.post('/shoutout', [
  ...
  db.qRun(
    'INSERT INTO messages (id, group_id, user_id, content, type) VALUES (?, ?, ?, ?, ?)',
    [id, group_id, req.userId, shoutContent, 'shoutout']
  );
