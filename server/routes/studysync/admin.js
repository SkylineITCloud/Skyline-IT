...
router.get('/users', (req, res) => {
  ...
  const users = db.q(
    'SELECT id, email, name, course, study_method, streak, total_hours, weekly_hours, is_online, last_seen, created_at FROM users ORDER BY created_at DESC LIMIT ? OFFSET ?', [limit, offset]
  );
...
router.post('/users', [
  ...
  db.qRun('INSERT INTO users (id, email, password_hash, name, course, study_method) VALUES (?, ?, ?, ?, ?, ?)',
